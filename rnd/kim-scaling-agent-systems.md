                                         ...
                                         1 Google Research, 2 Google DeepMind, 3 Massachusetts Institute of Technology, † Corresponding Author
...
arXiv:2512.08296v2 [cs.AI] 17 Dec 2025
                                         ...
                                         yields diminishing or negative returns ( 𝛽ˆ=−0.404, 𝑝<0.001) once single-agent baselines exceed an
...
• Quantitative scaling principles and architecture alignment: We derive a mixed-effects model
  (𝑅2 =0.524) using empirical coordination metrics—efficiency ( 𝐸 𝑐 ), error amplification ( 𝐴𝑒 ), and
...
reasons, and acts within the shared environment via iterative feedback.
    Formally, each agent 𝑎𝑖 is defined as a tuple 𝑆𝑖 = ( Φ𝑖 , A 𝑖 , 𝑀𝑖 , 𝜋𝑖 ), where:

• Φ𝑖 is the reasoning policy (typically an LLM)
• A 𝑖 = {ToolCall( 𝑡, 𝜃) : 𝑡 ∈ T , 𝜃 ∈ Θ𝑡 } is the action space consisting of tool usage, where T is the set of
...
• 𝜋𝑖 : H → A 𝑖 is the decision function mapping observation histories to actions
...
to produce the next tool call 𝛼𝑖,𝑡+1 .
    At timestep 𝑡 , agent 𝑎𝑖 selects an action 𝛼𝑖,𝑡 ∈ A 𝑖 according to:

                             𝛼𝑖,𝑡 = 𝜋𝑖 ( ℎ𝑖,𝑡 ) ,   𝑜𝑖,𝑡 = 𝐸 ( 𝛼𝑖,𝑡 ) ,   ℎ𝑖,𝑡+1 = 𝑓𝑖 ( ℎ𝑖,𝑡 , 𝛼𝑖,𝑡 , 𝑜𝑖,𝑡 ) ,

where 𝐸 denotes the environment and ℎ𝑖,0 = { 𝑠0 } contains the initial task specification. The history
update function 𝑓𝑖 : H × A 𝑖 × O → H appends the new action-observation pair to the agent’s
...
when optimal performance substantially benefits from adaptive interaction. Formally, if 𝜏 = {( 𝑎𝑡 , 𝑜𝑡 )}𝑇𝑡=0
...
complexity measured in terms of LLM calls, coordination overhead, and parallelization potential.
 ...
 Interaction Type
                             Agent                     Aggregation                   Majority Voting           Sub-Agent   Sub-Agent      Sub-Agent    Sub-Agent    Sub-Agent     Sub-Agent
...
                                                  Towards a Science of Scaling Agent Systems
                                 ...
                                 BrowseComp-Plus                                                             Finance Agent
                               -35%     +9%       +0%         +6%                                     +57%       +75%        +81%     +73%
                   0.5                                                               0.8
                                                                                     0.7
                   0.4                                                               0.6
Accuracy (%)
...
Δmax = 0.023 (computed as max𝑖, 𝑗 | 𝛽ˆarch,𝑖 − 𝛽ˆarch, 𝑗 | across families 𝑖, 𝑗 ∈ {OpenAI, Google, Anthropic}),
...
                                  Towards a Science of Scaling Agent Systems
 ...
 • Coordination efficiency 𝐸 𝑐 = 𝑆/(𝑇 /𝑇SAS ) (success normalized by relative turn count): normalizes
...
advantage ( 𝛽ˆ = −0.114, 95% CI: [−0.186, −0.042], 𝑝 = 0.002). The mechanism operates through
...
SAS). On Workbench, where multi-agent overhead is less tolerable (efficiency degrades from 𝐸 𝑐 = 0.466
for SAS to 𝐸 𝑐 = 0.074 for Hybrid, the largest relative drop across benchmarks), Anthropic’s best variant
...
defined in Appendix C, and 4) empirically measured coordination metrics from Table 5 (efficiency 𝐸 𝑐 ,
...
a quadratic term ( 𝐼 2 ) to test for non-linear capability scaling, and log-transformed tool count and
...
interactions, each motivated by observed patterns: 𝐸 𝑐 × 𝑇 tests whether efficiency penalties compound
...
overfitting.
   The complete functional form is:
                     ...
                     𝑃 = 𝛽0 + 𝛽1 ( 𝐼 − ¯𝐼 ) + 𝛽2 ( 𝐼 − ¯𝐼 ) 2 + 𝛽3 log(1 + 𝑇 ) + 𝛽4 log(1 + 𝑛𝑎 )
                             + 𝛽5 log(1 + 𝑂%) + 𝛽6 𝑐 + 𝛽7 𝑅 + 𝛽8 𝐸 𝑐 + 𝛽9 log(1 + 𝐴𝑒 )
                             + 𝛽10 𝑃SA + 𝛽11 ( 𝐼 × 𝐸 𝑐 ) + 𝛽12 ( 𝐴𝑒 × 𝑃SA )
                             + 𝛽13 ( 𝑂% × 𝑇 ) + 𝛽14 ( 𝑅 × 𝑛𝑎 ) + 𝛽15 ( 𝑐 × 𝐼 )
                             + 𝛽16 ( 𝐸 𝑐 × 𝑇 ) + 𝛽17 ( 𝑃SA × log(1 + 𝑛𝑎 ))
...
under alternative specifications (Δ𝑅2CV < 0.01). We validate model complexity through five-fold
cross-validation with experiment-level holdout, which yields 𝑅2CV = 0.524 (±0.033 SD), mean absolute
...
across folds (coefficient of variation < 18% for all | 𝛽ˆ| > 0.05), indicates that the 20 parameters are
justified by predictive power rather than overfitting. This model substantially outperforms simpler
alternatives using only architectural labels ( 𝑅2CV = 0.43) or intelligence alone (𝑅2CV = 0.28), as shown
...
bootstrap SE < 0.015 for all | 𝛽ˆ| > 0.1), and residual diagnostics satisfy normality (Shapiro–Wilk
𝑝 = 0.412) and homoscedasticity (Breusch–Pagan 𝑝 = 0.298), with residual standard error 𝜎       ˆ = 0.118.
We evaluated regularized alternatives: Lasso (10-fold CV for 𝜆 selection) retained 16 of 20 predictors
with 𝑅2CV = 0.506; Ridge achieved 𝑅2CV = 0.509. Given minimal improvement and the interpretability
benefits of the full model, we retain the unregularized specification.


The Efficiency-Tools Interaction Dominates Multi-Agent Performance ( 𝛽ˆ = −0.267, 𝑝 < 0.001).
Among the critical interactions, the efficiency-tools trade-off exhibits the second-largest effect size:
𝛽ˆ𝐸𝑐 ×𝑇 = −0.267 (95% CI: [−0.355, −0.178], 𝑝 < 0.001). This interaction reveals that tool-heavy tasks
suffer disproportionately from multi-agent inefficiency. Empirically, single-agent systems achieve
𝐸 𝑐 = 0.466 (Table 5), while multi-agent architectures range from 𝐸 𝑐 = 0.074 (hybrid) to 𝐸 𝑐 = 0.234
...
interpretability):
                                                    (
                                                      −1.99 (single-agent, 𝐸 𝑐 = 0.466)
                  Δ𝑃efficiency = −0.267 × 𝐸 𝑐 × 𝑇 =
                                                      −0.32 (multi-agent, 𝐸 𝑐 = 0.074)
...
architectures paradoxically more effective. The effect size ( 𝛽ˆ = −0.267) is approximately 1.6× larger
...
( 𝛽ˆ = −0.022, 𝑝 = 0.441) nor its interaction with tool count ( 𝐴𝑒 × 𝑇 : 𝛽ˆ = −0.019, 𝑝 = 0.506)
...
efficiency ( 𝐸 𝑐 ) and overhead (𝑂%)—rather than error propagation per se. Independent architecture’s
...
but this effect is subsumed by the efficiency metric ( 𝐸 𝑐 = 0.234 for Independent vs. 𝐸 𝑐 = 0.466 for
SAS).
...
performance. The scaling law reveals this overhead interacts with tool count ( 𝛽ˆ𝑂%×𝑇 = −0.162,
𝑝 < 0.001), creating a compounding cost for complex tasks. For hybrid architecture (𝑂% = 515)
on workbench (𝑇 = 16), the negative interaction ( 𝛽ˆ13 = −0.162, 𝑝 < 0.001) compounds overhead
...
Intelligence Shows Linear Positive Effect ( 𝛽ˆ𝐼 = 0.171, 𝑝 = 0.001). After centering intelligence
...
The quadratic term ( 𝐼 2 ) is not significant ( 𝑝 = 0.509), indicating that capability scaling follows a
...
with agent count ( 𝛽ˆ𝑅 × 𝑛𝑎 = 0.047, 95% CI: [0.019, 0.075], 𝑝 = 0.001), suggesting redundancy offers
...
compared to overhead penalties (| 𝛽ˆ𝑂%×𝑇 | = 0.162, 3.4× larger) and efficiency losses (| 𝛽ˆ𝐸𝑐 ×𝑇 | =
...
substantially improves predictive power.
                 ...
                 Model Specification                           𝑅2train     𝑅2CV         AIC   Parameters
...
metrics (efficiency, overhead, error amplification, redundancy, message density) achieves 20% improvement in 𝑅2CV over
...
the decision boundary between single-agent and multi-agent is:
                                    ...
                                    𝛽ˆ4        0.052
                            ∗
                           𝑃SA =−          ≈         = 0.129        (in standardized units),
                                    𝛽ˆ17       0.404
...
and empirical coordination metrics (𝑅2train = 0.613, 𝑅2CV = 0.524, 𝑛 = 180, AIC=−201.2). Intelligence
is mean-centered (¯𝐼 = 56.9) to address multicollinearity between 𝐼 and 𝐼 2 (VIF reduced from 200 to
1.1). Model uses 5-fold cross-validation. Non-significant terms ( 𝑝 > 0.05) indicated with †.
  ...
  Intelligence ( 𝐼 − ¯𝐼 )          0.171        [0.070, 0.272]        0.001      Linear capability effect
  Intelligence2 (( 𝐼 − ¯𝐼 ) 2 )    0.007       [−0.013, 0.026]        0.509†     Quadratic capability (not significant)
  ...
  Efficiency ( 𝐸 𝑐 )               −0.043      [−0.078, −0.007]       0.021      Coordination efficiency
  log(1 + 𝐴𝑒 )                     −0.022       [−0.077, 0.034]       0.441†     Error amplification
  Critical Interactions
  𝑃SA × log(1 + 𝑛𝑎 )               −0.404      [−0.557, −0.252]       <0.001     Baseline paradox
  𝐸𝑐 × 𝑇                           −0.267      [−0.355, −0.178]       <0.001     Efficiency-tools trade-off
  𝑂% × 𝑇                           −0.162      [−0.241, −0.083]       <0.001     Overhead scales with task complexity
  𝐴𝑒 × 𝑇                           −0.019       [−0.075, 0.037]       0.506†     Error propagation in tool-rich systems
  𝑅 × 𝑛𝑎                            0.047        [0.019, 0.075]       0.001      Redundancy benefit with scale
  𝐼 × 𝐸𝑐                           −0.022       [−0.075, 0.030]       0.404†     Capability-efficiency
  𝐴𝑒 × 𝑃SA                         −0.065       [−0.146, 0.015]       0.114†     Error-baseline
  𝑐×𝐼                              −0.011       [−0.057, 0.034]       0.626†     Communication-capability
  𝐼 × log(1 + 𝑇 )                  −0.069       [−0.138, 0.000]       0.053†     Capability-tools
...
instance runs). All systems matched for total reasoning tokens (mean 𝜇 = 4, 800 per trial).
     ...
     Efficiency ( 𝐸 𝑐 )           0.466           0.234                 0.132             0.120           0.074
...
logarithmic relationship with message density across all architectures:
                            ...
                            𝑆 = 0.73 + 0.28 ln( 𝑐) ,     𝑅2 = 0.68,       𝑝 < 0.001,
...
Accuracy
           ...
           0.0                                                                                                         0.0
                 ...
                 All High-intelligence                               All Low-intelligence                                    All High-intelligence            All Low-intelligence
                 High Orch. + Low Sub-agents                         Low Orch. + High Sub-agents                             Mixed intelligence
...
(ii) Optimal band (200% < 𝑂 < 300% overhead): highest success–cost ratio ( 𝐸 𝑐 ≈ 0.16), dom-
inated by Centralized and Decentralized, with strong error absorption; (iii) Over-coordination
(𝑂 > 400% overhead): Hybrid runs with reduced efficiency ( 𝐸 𝑐 ≈ 0.11), protocol complexity intro-
...
                                                     Towards a Science of Scaling Agent Systems
        ...
        (a)                          Gemini 2.0 Flash                      (b)                               Gemini 2.5 Pro
                        35
...
Y. Yu, H. Li, Z. Chen, Y. Jiang, Y. Li, J. W. Suchow, D. Zhang, and K. Khashanah. Finmem: A
  performance-enhanced LLM trading agent with layered memory and character design. IEEE
  Transactions on Big Data, 2025.

G. Zhang, L. Niu, J. Fang, K. Wang, L. Bai, and X. Wang. Multi-agent architecture search via
  agentic supernet. In Forty-second International Conference on Machine Learning, 2025a. URL
...
support for large-context evaluation (e.g., “non-reasoning” checkpoints), our long-context estimates
  3 https://artificialanalysis.ai/evaluations/artificial-analysis-intelligence-index
...
Table 6 | Intelligence Index (non-agentic capability) for LLMs used in our experiments.
 ...
 Model                  Index   AA-LCR    HLE    MMLU-Pro      GPQA Diamond       AIME 25     LiveCode   SciCode   IFBench
    ...
    Claude 4.0 Sonnet    47       62∗      5∗         87             75               21        56∗        38∗       35∗
...
Table 7 | Out-of-sample validation metrics for GPT-5.2 on BrowseComp-Plus.
                     ...
                     Kendall’s 𝜏 (Ranking)                                   0.200            Weak
...
quadratic intelligence term, which is not significant after centering ( 𝛽ˆ𝐼 2 = 0.007, 𝑝 = 0.509). Instead,
...
principles derived from the training set generalize to higher-capability models.
 ...
 Capability Ceiling: 𝑃SA ≥ 0.45 ⇒ diminish-           𝑃SA = 0.45; best MAS gain = +6.7%                    ✓
 ing MAS returns ( 𝛽ˆ = −0.404)
 Independent MAS Degradation: Indepen-                Independent = 0.35 vs SAS = 0.45                     ✓
 dent architecture underperforms SAS                  (−22.2%)
 Optimal      Architecture:        Central-           Best: Centralized & Decentralized (tied at           ✓
 ized/Decentralized excel on BrowseComp-              0.48)
 Plus
 Hybrid Overhead: 515% overhead de-                   Hybrid = 0.39 vs SAS = 0.45 (−13.3%)                 ✓
 grades performance
 BrowseComp Pattern: Decentralized > Cen-             Decentralized = 0.48, Centralized = 0.48          Partial†
...
the over-prediction likely reflects limitations in linear extrapolation of the capability effect ( 𝛽ˆ𝐼 = 0.171)
...
GPT-5.2 ( 𝑃ˆSAS = 0.673), yet empirically MAS-Centralized and MAS-Decentralized achieve superior
performance (0.48 vs. 0.45). This discrepancy does not arise from the quadratic intelligence term,
which is not significant after mean-centering ( 𝛽ˆ𝐼 2 = 0.007, 𝑝 = 0.509). Rather, the over-prediction
reflects the linear intelligence effect ( 𝛽ˆ𝐼 = 0.171) extrapolating beyond its training range, potentially
...
higher-capability models.
  ...
  SAS                     0.673     0.450     +49.5%∗
                                                                 BrowseComp pattern (Decent. ≥ Cent.)          Partial†
  MAE (MAS only)                                0.034
                                                                 Total                                           4/5
  MAE (Overall)                                 0.071
...
Table 10 | Domain complexity scores and task characteristics.
  ...
  Finance Agent            0.407     Moderate decomposability; structured domains amenable to localised
                                     agent reasoning
  PlanCraft                0.419     High sequential dependencies; constraint satisfaction requiring or-
